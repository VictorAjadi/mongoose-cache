...
class MongoExpressionEvaluator {
    ...
    private static initializeDiacriticMap(): void {
    ...
    public static evaluateExpression(document: any, expr: any, root?: any): boolean {
    ...
    private static evaluateOperator(operator: string, operand: any, context: EvaluationContext): boolean {
    ...
    private static getOperatorEvaluator(operator: string): Function {
        switch (operator) {
            // Comparison operators
            case '$eq': return (op: any, ctx: EvaluationContext) => this.evaluateComparison('$eq', op, ctx);
            case '$ne': return (op: any, ctx: EvaluationContext) => this.evaluateComparison('$ne', op, ctx);
            case '$gt': return (op: any, ctx: EvaluationContext) => this.evaluateComparison('$gt', op, ctx);
            case '$gte': return (op: any, ctx: EvaluationContext) => this.evaluateComparison('$gte', op, ctx);
            case '$lt': return (op: any, ctx: EvaluationContext) => this.evaluateComparison('$lt', op, ctx);
            case '$lte': return (op: any, ctx: EvaluationContext) => this.evaluateComparison('$lte', op, ctx);
            case '$in': return (op: any, ctx: EvaluationContext) => this.evaluateComparison('$in', op, ctx);
            case '$nin': return (op: any, ctx: EvaluationContext) => this.evaluateComparison('$nin', op, ctx);

            // Logical operators
            case '$and': return (op: any, ctx: EvaluationContext) => this.evaluateLogical('$and', op, ctx);
            case '$or': return (op: any, ctx: EvaluationContext) => this.evaluateLogical('$or', op, ctx);
            case '$not': return (op: any, ctx: EvaluationContext) => this.evaluateLogical('$not', op, ctx);
            case '$nor': return (op: any, ctx: EvaluationContext) => this.evaluateLogical('$nor', op, ctx);

            // Element operators
            case '$exists': return (op: any, ctx: EvaluationContext) => this.evaluateExists(op, ctx);
            case '$type': return (op: any, ctx: EvaluationContext) => this.evaluateType(op, ctx);

            // Array operators
            case '$all': return (op: any, ctx: EvaluationContext) => this.evaluateArrayOperator('$all', op, ctx);
            case '$elemMatch': return (op: any, ctx: EvaluationContext) => this.evaluateArrayOperator('$elemMatch', op, ctx);
            case '$size': return (op: any, ctx: EvaluationContext) => this.evaluateArrayOperator('$size', op, ctx);

            // Text search
            case '$text': return (op: any, ctx: EvaluationContext) => this.evaluateFullTextSearch(op, ctx);
            case '$regex': return (op: any, ctx: EvaluationContext) => this.evaluateRegexSearch(op, ctx);

            // Conditional operators
            case '$cond': return (op: any, ctx: EvaluationContext) => !!this.evaluateConditional(op, ctx);
            case '$ifNull': return (op: any, ctx: EvaluationContext) => !!this.evaluateIfNull(op, ctx);
            case '$switch': return (op: any, ctx: EvaluationContext) => !!this.evaluateSwitch(op, ctx);
            ...
            case '$exp':
            case '$trunc':
                return (op: any, ctx: EvaluationContext) => this.evaluateArithmetic(operator, op, ctx) !== null;
            ...
            case '$replaceOne':
            case '$replaceAll':
                return (op: any, ctx: EvaluationContext) => this.evaluateString(operator, op, ctx) !== null;
            ...
            case '$dateToParts':
            case '$dateFromParts':
                return (op: any, ctx: EvaluationContext) => this.evaluateDate(operator, op, ctx) !== null;

            // Comparison aggregation
            case '$cmp': return (op: any, ctx: EvaluationContext) => this.evaluateCmp(op, ctx) !== null;
    ...
    private static evaluateComparison(op: string, operand: any, context: EvaluationContext): boolean {
        const values = Array.isArray(operand)
            ? operand.map(val => this.resolveValue(val, context))
            : [this.resolveValue(operand, context)];

        if (values.length < 2 && op !== '$in' && op !== '$nin') return true;
    ...
    private static evaluateLogical(op: string, operand: any, context: EvaluationContext): boolean {
        switch (op) {
            case '$and':
    ...
    private static evaluateExists(operand: any, context: EvaluationContext): boolean {
    ...
    private static evaluateType(operand: any, context: EvaluationContext): boolean {
    ...
    private static evaluateArrayOperator(op: string, operand: any, context: EvaluationContext): boolean {
        const value = context.document;

        switch (op) {
            case '$all':
                if (!Array.isArray(value) || !Array.isArray(operand)) return false;
                return operand.every(searchVal =>
                    value.some(docVal => this.fastEquals(docVal, searchVal))
                );

            case '$elemMatch':
                if (!Array.isArray(value)) return false;
                return value.some(item =>
                    this.evaluateExpression(item, operand, context.root)
                );
    ...
    private static evaluateFullTextSearch(operand: any, context: EvaluationContext): boolean {
    ...
    private static parseSearchTerms(searchText: string): SearchTerms {
        const terms: SearchTerms = { include: [], exclude: [], phrases: [] };
    ...
    private static removeDiacriticsFast(str: string): string {
    ...
    private static evaluateConditional(operand: any, context: EvaluationContext): any {
        if (!Array.isArray(operand) || operand.length !== 3) return null;
    ...
    private static evaluateIfNull(operand: any, context: EvaluationContext): any {
    ...
    private static evaluateSwitch(operand: any, context: EvaluationContext): any {
    ...
    private static evaluateArithmetic(op: string, operand: any, context: EvaluationContext): number | null {
        const values = Array.isArray(operand)
            ? operand.map(val => this.resolveValue(val, context))
            : [this.resolveValue(operand, context)];
    ...
    private static evaluateString(op: string, operand: any, context: EvaluationContext): string | number | string[] | null {
        const values = Array.isArray(operand)
            ? operand.map(val => this.resolveValue(val, context))
            : [this.resolveValue(operand, context)];
    ...
    private static evaluateDate(operator: string, value: any, context: EvaluationContext): any {
        const resolve = (v: any) => this.resolveValue(v, context);
        ...
        const getUnitMillis = (unit: string): number => {
        ...
        const addDate = (date: Date, amount: number, unit: string): Date => {
        ...
        switch (operator) {
            ...
            case '$dateTrunc': {
                ...
                switch (unit) {
                    case 'year': truncated.setUTCMonth(0, 1); truncated.setUTCHours(0, 0, 0, 0); break;
                    case 'month': truncated.setUTCDate(1); truncated.setUTCHours(0, 0, 0, 0); break;
                    case 'day': truncated.setUTCHours(0, 0, 0, 0); break;
                    case 'hour': truncated.setUTCMinutes(0, 0, 0); break;
                    case 'minute': truncated.setUTCSeconds(0, 0); break;
                    case 'second': truncated.setUTCMilliseconds(0); break;
                }
                if (timezone) return new Date(truncated.toLocaleString('en-US', { timeZone: timezone }));
                return truncated;
            }
            ...
            case '$dateFromParts': {
                const {
                    ...
                    second = 0, millisecond = 0, timezone
                } = value;
    ...
    private static evaluateCmp(operand: any, context: EvaluationContext): number | null {
        if (!Array.isArray(operand) || operand.length !== 2) return null;
    ...
    private static resolveValue(value: any, context: EvaluationContext): any {
        if (value === null || value === undefined) return value;

        // Handle string expressions like $ROOT, $CURRENT, or $field
        if (typeof value === 'string' && value.startsWith('$')) {
            if (value === '$ROOT') return context.root;
            if (value === '$CURRENT') return context.document;
    ...
    private static getBSONType(value: any): string {
    ...
    private static bsonTypeNumberToString(typeNumber: number): string {
