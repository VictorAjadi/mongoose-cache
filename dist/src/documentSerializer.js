...
export class DocumentSerializer {
    ...
    static MONGOOSE_INTERNALS = new Set([
     ...
     */
    static serialize(value, depth = 0, seen = new WeakSet()) {
        ...
        if (value.$__ || value._doc) {
            ...
            if (typeof value.toObject === 'function') {
                try {
                    return this.serialize(value.toObject({
                        virtuals: false,
                        getters: false,
                        versionKey: false,
                        depopulate: true,
     ...
     */
    static deserialize(value, depth = 0) {
        if (depth > 50) {
     ...
     */
    static rehydrateObjectIds(data) {
        if (!data)
            return data;
     ...
     */
    static needsSerialization(value) {
        if (value === null || value === undefined)
            return false;
        const type = typeof value;
        // Primitives are safe
     ...
     */
    static estimateSize(value, depth = 0) {
        if (depth > 20)
            return 100; // Assume deep objects are ~100 bytes each
        if (value === null || value === undefined)
            return 4;
        const type = typeof value;
        if (type === 'string')
        ...
        if (type === 'object') {
            ...
            for (let i = 0; i < keys.length; i++) {
                const key = keys[i];
                if (DocumentSerializer.MONGOOSE_INTERNALS.has(key))
                    continue;
