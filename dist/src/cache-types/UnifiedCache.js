...
export class UnifiedCache {
    ...
    isRedisConfigured() {
        ...
        return Boolean(this.config.redis?.host !== DEFAULT_CONFIG.redis.host ||
        ...
        for (let attempt = 1; attempt <= MAX_INIT_ATTEMPTS; attempt++) {
            try {
                ...
                if (attempt < MAX_INIT_ATTEMPTS) {
                    if (this.config.debug) {
                        console.warn(`[Redis] Connection attempt ${attempt} failed, ` +
                ...
                if (this.config.debug) {
                    console.error(`[Redis] Attempt ${attempt} error: ${error.message}`);
                }
                if (attempt >= MAX_INIT_ATTEMPTS) {
                    ...
                    if (this.config.debug) {
                        console.error('[Redis] Initialization failed after all attempts. ' +
     ...
     */
    async ensureRedisReady(timeoutMs = 10000) {
        if (!this.useRedis) {
            return false;
        }
     ...
     */
    generateKey(modelName, op, query, options) {
        ...
        if (query && typeof query === 'object') {
            if (Array.isArray(query)) {
                ...
                if (this.config.useCryptoHash) {
                    ...
                    const pipeline = query.map((stage, idx) => `${idx}:${JSON.stringify(stage)}`).join('|');
     ...
     */
    addToIndexes(cacheKey, modelName, query) {
     ...
     */
    async invalidateByQuery(modelName, updateQuery) {
        try {
            ...
            if (this.config.debug && invalidatedCount > 0) {
                console.log(`[Cache] Invalidated ${invalidatedCount} entries for ` +
    ...
    async invalidateModel(modelName) {
        try {
            ...
            if (this.config.debug && invalidatedCount > 0) {
                console.log(`[Cache] Invalidated ${invalidatedCount} entries for model ${modelName}`);
            }
            return invalidatedCount;
        }
        catch (error) {
    ...
    async getKeysByPattern(pattern) {
        try {
            ...
            if (cache instanceof RedisAdapter) {
                ...
                const keys = await cache.client?.keys(fullPattern);
                return keys || [];
            }
            else {
