...
export class MemoryCache extends EventEmitter {
    ...
    lastEvictionTime = 0;
    MIN_EVICTION_INTERVAL = 10000; // 10 seconds
    ...
    constructor(config) {
        ...
        this.cache = new HyperHashMap(this.config.maxKeys);
        this.indexes = new HyperHashMap(1024);
    ...
    calculateMaxSize() {
        ...
        if (this.debugMode) {
            console.log(`[MemoryCache] Component Target: ${(this.maxSizeBytes / 1048576).toFixed(2)}MB ` +
    ...
    startMemoryMonitoring() {
        this.memoryCheckTimer = setInterval(() => {
            ...
            if (isInternalPressure || isProcessPressure) {
                ...
                if (this.debugMode) {
                    console.warn(`[MemoryCache] PRESSURE: Internal ${internalUsagePercent.toFixed(1)}% | ` +
    ...
    evictToTarget() {
        ...
        if (this.debugMode) {
            console.log(`Evicting to free ${(bytesToFree / 1048576).toFixed(2)}MB ` +
    ...
    invalidateByQuery(modelName, updateQuery) {
        if (!this.config.enableSmartInvalidation) {
