...
class JSONSchemaValidator {
    ...
    static FORMAT_VALIDATORS = new Map();
    static TYPE_CACHE = new Map();
    static {
        this.initializeFormatValidators();
    }
    static initializeFormatValidators() {
    ...
    static validateJsonSchema(value, schema) {
        try {
    ...
    static validateSchema(value, schema, path, visited) {
        ...
        if (schema.enum !== undefined) {
            if (!this.validateEnum(value, schema.enum))
                return false;
        }
        if (schema.const !== undefined) {
            if (!this.fastEquals(value, schema.const))
                return false;
        }
        const valueType = typeof value;
        if (valueType === 'string') {
            if (!this.validateString(value, schema))
                return false;
        }
        else if (valueType === 'number') {
            if (!this.validateNumber(value, schema))
                return false;
        }
        else if (Array.isArray(value)) {
            if (!this.validateArray(value, schema, path, visited))
                return false;
        }
        else if (value !== null && valueType === 'object') {
            if (!this.validateObject(value, schema, path, visited))
                return false;
        }
        if (schema.allOf !== undefined) {
            if (!this.validateAllOf(value, schema.allOf, path, visited))
                return false;
        }
        if (schema.anyOf !== undefined) {
            if (!this.validateAnyOf(value, schema.anyOf, path, visited))
                return false;
        }
        if (schema.oneOf !== undefined) {
            if (!this.validateOneOf(value, schema.oneOf, path, visited))
                return false;
        }
        if (schema.not !== undefined) {
            if (this.validateSchema(value, schema.not, path, visited))
                return false;
        }
        if (schema.if !== undefined) {
            if (!this.validateIfThenElse(value, schema, path, visited))
                return false;
        }
        return true;
    }
    static validateType(value, type) {
        ...
        for (let i = 0; i < types.length; i++) {
            if (types[i] === actualType)
                return true;
            if (types[i] === 'number' && actualType === 'integer')
                return true;
        }
        return false;
    }
    ...
    static validateEnum(value, enumValues) {
        for (let i = 0; i < enumValues.length; i++) {
            if (this.fastEquals(value, enumValues[i]))
                return true;
        }
        return false;
    }
    ...
    static validateArray(value, schema, path, visited) {
        const len = value.length;
        ...
        if (schema.uniqueItems === true) {
            if (!this.hasUniqueItemsFast(value))
                return false;
        }
    ...
    static validateObject(value, schema, path, visited) {
        ...
        if (schema.additionalProperties !== undefined) {
            ...
            for (let i = 0; i < keys.length; i++) {
                const key = keys[i];
                if (definedProps && definedProps.has(key))
                    continue;
    ...
    static validateAllOf(value, schemas, path, visited) {
    ...
    static validateAnyOf(value, schemas, path, visited) {
        for (let i = 0; i < schemas.length; i++) {
            if (this.validateSchema(value, schemas[i], path, visited))
                return true;
        }
    ...
    static validateOneOf(value, schemas, path, visited) {
        let validCount = 0;
        for (let i = 0; i < schemas.length; i++) {
            if (this.validateSchema(value, schemas[i], path, visited)) {
                validCount++;
    ...
    static validateIfThenElse(value, schema, path, visited) {
    ...
    static hasUniqueItemsFast(array) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (item === null || type === 'boolean' || type === 'number' || type === 'string') {
                ...
                for (let j = 0; j < objects.length; j++) {
                    if (this.deepEquals(item, objects[j]))
                        return false;
                }
                objects.push(item);
            }
    ...
    static deepEquals(a, b, visited = new WeakMap()) {
        if (a === b)
            return true;
        if (a == null || b == null)
            return a === b;
    ...
    static isValidDateTime(value) {
        if (value.length < 19 || value.length > 35)
            return false;
    ...
    static isValidIDNHostname(value) {
        if (value.length === 0 || value.length > 253)
            return false;
        const labels = value.split('.');
        for (const label of labels) {
    ...
    static isValidURIReference(value) {
        try {
    ...
    static isValidIRIReference(value) {
        try {
            new URL(value, 'http://example.com');
            return true;
        }
    ...
    static isValidURITemplate(value) {
        let depth = 0;
    ...
    static isValidJSONPointer(value) {
        if (value === '')
            return true;
        if (!value.startsWith('/'))
            return false;
    ...
    static isValidRelativeJSONPointer(value) {
        if (value.length === 0)
            return false;
        let i = 0;
        if (value[0] === '0') {
    ...
    static precompileSchema(schema) {
        if (!schema || typeof schema !== 'object')
            return;
        if (schema.pattern) {
            try {
