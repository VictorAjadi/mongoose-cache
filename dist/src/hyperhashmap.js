 ...
 */
export class HyperHashMap {
    keys; // Sparse array storing keys
    values; // Sparse array storing values
    distances; // Probe distance for each slot (0-255)
    capacity; // Current hash table size (always power of 2)
    size_ = 0; // Number of stored key-value pairs
    mask; // Bitmask for fast modulo (capacity - 1)
    maxDistance = 0; // Maximum probe distance seen (for search optimization)
    loadFactor = 0.75; // Threshold for triggering resize
    /**
    ...
    set(key, value) {
        ...
        while (true) {
            ...
            if (distance > this.distances[index]) {
                [insertKey, this.keys[index]] = [this.keys[index], insertKey];
                [insertValue, this.values[index]] = [this.values[index], insertValue];
