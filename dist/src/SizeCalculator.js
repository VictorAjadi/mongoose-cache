import { ObjectId, Binary, Long, Double, Int32, Decimal128, Timestamp, BSONRegExp, BSONSymbol, Code, MinKey, MaxKey, DBRef } from 'bson';
/**
...
export class SizeCalculator {
    ...
    static DOCUMENT_OVERHEAD = 5; // int32 size (4) + null terminator (1)
    static ELEMENT_OVERHEAD = 1; // type byte (1)
    static STRING_OVERHEAD = 5; // int32 length (4) + null terminator (1)
    static BINARY_OVERHEAD = 5; // int32 length (4) + subtype (1)
    static CODE_W_SCOPE_BASE = 8; // int32 total_size (4) + int32 code_length (4)
    static DBREF_BASE = 12; // Overhead for $ref, $id, $db field names and type bytes
    static FIELD_NAME_TERMINATOR = 1; // null terminator for field names
    static CSTRING_TERMINATOR = 1; // null terminator for C strings
    // Performance optimization cache
    static UTF8_CACHE = new Map();
    static FIELD_CACHE = new Map();
    visited = new WeakSet();
    depth = 0;
     ...
     */
    calculateSize(value, isRoot, fieldName) {
     ...
     */
    handleBSONSpecialTypes(value, isRoot, fieldName) {
        // ObjectId - use type constant
        if (value instanceof ObjectId) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.OBJECT_ID, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.OBJECT_ID], isRoot, fieldName);
        }
        // Date - use type constant
        if (value instanceof Date) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.DATE, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.DATE], isRoot, fieldName);
        }
        ...
        if (value instanceof Long) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.LONG, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.LONG], isRoot, fieldName);
        }
        if (value instanceof Double) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.DOUBLE, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.DOUBLE], isRoot, fieldName);
        }
        if (value instanceof Int32) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.INT, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.INT], isRoot, fieldName);
        }
        if (value instanceof Decimal128) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.DECIMAL128, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.DECIMAL128], isRoot, fieldName);
        }
        // Timestamp - use type constant
        if (value instanceof Timestamp) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.TIMESTAMP, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.TIMESTAMP], isRoot, fieldName);
        }
        ...
        if (value instanceof MinKey) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.MIN_KEY, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.MIN_KEY], isRoot, fieldName);
        }
        if (value instanceof MaxKey) {
            return this.calculateTypeSize(SizeCalculator.BSON_TYPES.MAX_KEY, SizeCalculator.TYPE_SIZES[SizeCalculator.BSON_TYPES.MAX_KEY], isRoot, fieldName);
        }
     ...
     */
    handleObjectType(obj, isRoot, fieldName) {
     ...
     */
    calculateDocumentSize(doc, isRoot, parentField) {
        let size = 0;
     ...
     */
    calculateArraySize(arr, isRoot, fieldName) {
        let size = 0;
        // Add field overhead if this is not root
        if (!isRoot) {
     ...
     */
    calculateNumberSize(value, isRoot, fieldName) {
     ...
     */
    calculateStringSize(value, isRoot, fieldName) {
     ...
     */
    calculateSymbolSize(value, isRoot, fieldName) {
     ...
     */
    calculateBinarySize(dataLength, isRoot, fieldName) {
        const fieldOverhead = isRoot ? 0 : this.getFieldOverhead(fieldName);
     ...
     */
    calculateRegExpSize(pattern, flags, isRoot, fieldName) {
        const fieldOverhead = isRoot ? 0 : this.getFieldOverhead(fieldName);
     ...
     */
    calculateCodeSize(code, isRoot, fieldName) {
        const fieldOverhead = isRoot ? 0 : this.getFieldOverhead(fieldName);
     ...
     */
    calculateDBRefSize(dbref, isRoot, fieldName) {
        const fieldOverhead = isRoot ? 0 : this.getFieldOverhead(fieldName);
     ...
     */
    handleMap(map, isRoot, fieldName) {
        const obj = {};
    ...
    static validateSizeCalculation(document, expectedSize) {
